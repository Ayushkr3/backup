...
BoxCollider::BoxCollider(std::vector<Vertex>& vertices, std::vector<NormalPerObject> Normals, ImGuiContext * ctx) :vert(vertices), ObjectsNormals{Normals},cxt(ctx) {
	CalcAxis(Normals);
	ImGui::SetCurrentContext(ctx);
}
std::vector<Collision::Projection> BoxCollider::CalcProjection(std::vector<DirectX::XMFLOAT3> normals) {
...
BoxCollider* CreateBoxCollider(std::vector<Vertex>& vertices, std::vector<NormalPerObject> Normals)
{
	return new BoxCollider(vertices,Normals);
}
